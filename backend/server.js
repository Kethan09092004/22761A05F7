...
async function refreshToken() {
  try {
    const response = await axios.post(`${TEST_SERVER_URL}/auth`, { // Verify this endpoint
      ...
      ownerEmail: 'kethan09092004@gmail.com', // Double-check this email
      rollNo: '22761A05F7',
    });
    console.log('Full auth response:', response.data);
    AUTH_TOKEN = response.data.access_token || response.data.token; // Handle common token fields
    if (!AUTH_TOKEN) {
    ...
    console.log('New token:', AUTH_TOKEN);
  } catch (error) {
...
async function fetchPostComments(postId) {
  ...
  if (!comments) {
    try {
      const response = await axiosInstance.get(`/posts/${userId}/comments`);
